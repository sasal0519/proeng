...
class SymbolPalette(QToolBox):
    ...
    def _apply_list_style(self, lw):
        t = T()
        lw.setStyleSheet(f"""
            QListWidget {{ background: {t["bg_app"]}; border: none; padding: 4px; }}
            QListWidget::item {{
                color: {t["text"]};
                font-size: 11px;
                font-weight: bold;
...
class TextAnnotation(QGraphicsItem):
    """Texto livre no canvas — movivel, com portas para conectar pipes."""
    ...
    def __init__(self, scene, pos, text=""):
        ...
        # Move, select, snap — igual equipamentos
        self.setFlags(
            QGraphicsItem.ItemIsMovable
            | QGraphicsItem.ItemIsSelectable
            | QGraphicsItem.ItemSendsGeometryChanges
        ...
        # Connector ports: left e right (igual SourceSinkHandle)
        self.edges = []
        self.ports = {}
        for pid, ppos in [("left", QPointF(-4, 0)), ("right", QPointF(4, 0))]:
            port = ConnectorPort(self, pid)
            port.setPos(ppos)
            self.ports[pid] = port
            port.setVisible(False)
        ...
        # Se nenhum texto foi passado, abre edicao
        if not text:
            QTimer.singleShot(100, self._start_inline_edit)
    ...
    def paint(self, painter, option, widget=None):
        ...
        for line in self._text.split("\n"):
            painter.drawText(QPointF(0, y + fm.height()), line)
            y += fm.height()

    # ── Edição ─────────────────────────────────────
    def _start_inline_edit(self):
        text, ok = QInputDialog.getMultiLineText(
            None, "Anotação de Texto", "Digite o texto:", self._text
        )
    ...
    def mouseDoubleClickEvent(self, event):
        if event.button() == Qt.LeftButton:
            text, ok = QInputDialog.getMultiLineText(
                None, "Editar Anotação", "Edite o texto:", self._text
            )
            if ok and text.strip():
                self._text = text.strip()
                self.prepareGeometryChange()
    ...
    def contextMenuEvent(self, event):
        ...
        rename_act = menu.addAction("✏️ Editar Texto")
...
class FlowsheetWidget(QWidget):
    def __init__(self):
        ...
        self.btn_text = QPushButton("T Texto")
        self.btn_text.setCheckable(True)
        self.btn_text.clicked.connect(self._add_text_mode)

        tb_lay.insertWidget(1, self.btn_pal)
        tb_lay.addWidget(self.btn_text)
    ...
    def _add_text_mode(self):
        """Cria uma anotação de texto no centro do canvas visível."""
        vp_center = self.canvas.viewport().rect().center()
        scene_pos = self.canvas.mapToScene(vp_center)
        annotation = TextAnnotation(self.scene, scene_pos)
...
class _FlowsheetModule(BaseModule):
    def __init__(self):
        ...
        self.help_text = (
            "PFD FLOWSHEET — Guia Rapido\n\n"
            "O PFD (Process Flow Diagram) e um diagrama de fluxo "
            "de processo industrial que representa equipamentos, "
            "tubulacoes e correntes de materia/energia. Inclui "
            "simulacao de balanco de massa com convergencia "
            "automatica para reciclos.\n\n"
            "COMO USAR:\n"
            "• Arraste equipamentos do painel esquerdo para a area "
            "de trabalho (bombas, reatores, trocadores, etc.).\n"
            "• Clique com o botao direito em um equipamento para "
            "adicionar terminais de entrada/saida, renomear ou "
            "configurar parametros de desempenho.\n"
            "• Conecte equipamentos clicando em um terminal de "
            "saida e arrastando ate um terminal de entrada.\n"
            "• Clique duas vezes em uma tubulacao (pipe) para abrir "
            "o editor de corrente e definir composicao e vazao.\n\n"
            "BALANCO DE MASSA:\n"
            "• A grade de resultados (painel inferior) exibe em "
            "tempo real os dados de cada corrente e equipamento.\n"
            "• O balanco e recalculado automaticamente ao editar "
            "dados de correntes ou alterar conexoes.\n"
            "• Suporte a reciclo e purga com convergencia iterativa.\n\n"
            "NAVEGACAO:\n"
            "• Use Ctrl + Scroll do mouse para zoom.\n"
            "• Arraste o fundo para navegar pelo diagrama.\n"
            "• Equipamentos podem ser movidos livremente; as "
            "tubulacoes se ajustam automaticamente."
        )
        layout = QVBoxLayout(self)
        layout.setContentsMargins(0, 0, 0, 0)
