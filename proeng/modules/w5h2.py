...
class W5H2Node(QGraphicsItem):
    ...
    def __init__(self, node_id, text, node_type, signals, zoom):
        ...
        ff_content = t.get("font_family_content", "Segoe UI")
        if self.node_type == "ROOT":
            self._font = QFont(ff, int(14 * zoom), QFont.Bold)
            self.base_w, self.base_h = 240 * zoom, 80 * zoom
        elif self.node_type == "WHAT":
            self._font = QFont(ff, int(12 * zoom), QFont.Bold)
            self.base_w, self.base_h = 220 * zoom, 70 * zoom
        else:
            self._font = QFont(ff_content, int(10 * zoom), QFont.Normal)
            ...
            if self.node_type in ["ROOT", "WHAT"]:
                ...
                if _is_nb(t):
                    ...
                    if t.get("border_radius", 0) > 0:
                        ...
                        painter.drawRect(add_rect)
                painter.setPen(QColor(t.get("node_text", "#FFF")))
                ff = t.get("font_family", "'Segoe UI', sans-serif")
                painter.setFont(QFont(ff.replace("'", ""), int(14 * self.zoom), QFont.Bold))
...
class _W5H2Module(BaseModule):
    ...
    def __init__(self):
        ...
        self.help_text = (
            "PLANO DE ACAO 5W2H — Guia Rapido\n\n"
            "O 5W2H e uma metodologia de planejamento que define "
            "acoes de forma clara e completa, respondendo a 7 perguntas "
            "fundamentais: O que (What), Por que (Why), Quem (Who), "
            "Onde (Where), Quando (When), Como (How) e Quanto custa "
            "(How much).\n\n"
            "COMO USAR:\n"
            "• Clique duas vezes na caixa raiz (vermelha) para definir "
            "o nome do projeto ou meta principal.\n"
            "• Clique no botao (+) ao lado da raiz ou de uma acao "
            "para adicionar uma nova Acao (O QUE?). Os 6 detalhes "
            "(POR QUE, QUEM, ONDE, QUANDO, COMO, QUANTO) sao "
            "criados automaticamente.\n"
            "• Clique duas vezes em qualquer caixa colorida para "
            "preencher as informacoes de cada campo.\n"
            "• Use o botao (-) que aparece ao passar o mouse sobre "
            "uma acao para excluir a acao e todos os seus detalhes.\n\n"
            "ESTRUTURA HIERARQUICA:\n"
            "• Nivel 1 (Raiz): Projeto / Meta principal\n"
            "• Nivel 2 (Acoes): O QUE fazer (tarefas concretas)\n"
            "• Nivel 3 (Detalhes): 6 campos de cada acao\n\n"
            "NAVEGACAO:\n"
            "• Use Ctrl + Scroll do mouse para zoom.\n"
            "• Arraste o fundo para navegar pelo diagrama.\n"
            "• Os baloes ajustam seu tamanho ao conteudo inserido."
        )
