...
def draw_canvas_icon(painter, icon_name, rect):
    ...
    if icon_name == "just":
        ...
        painter.drawLine(QPointF(x+w*0.1, y+h*0.6), QPointF(x-w*0.1, y+h*0.9))
    elif icon_name == "obj":
        ...
        painter.drawLine(QPointF(x+w*0.3, y+h*0.3), QPointF(x+w*0.7, y+h*0.3))
        painter.drawLine(QPointF(x+w*0.3, y+h*0.5), QPointF(x+w*0.7, y+h*0.5))
        painter.drawLine(QPointF(x+w*0.3, y+h*0.7), QPointF(x+w*0.5, y+h*0.7))
    elif icon_name == "stk":
        ...
        painter.drawLine(QPointF(x+w*0.2, y+h*0.8), QPointF(x+w*0.8, y+h*0.8))
    elif icon_name == "ent":
        ...
        painter.drawLine(QPointF(x+w*0.15, y+h*0.15), QPointF(x+w*0.85, y+h*0.85))
    elif icon_name == "risc":
        ...
        painter.drawLine(QPointF(x+w*0.15, y+h*0.47), QPointF(x+w*0.4, y+h*0.47))
        painter.drawLine(QPointF(x+w*0.55, y+h*0.47), QPointF(x+w*0.8, y+h*0.47))
    elif icon_name == "cst":
        painter.setFont(QFont("Segoe UI", int(h*0.8), QFont.Bold))
...
class CanvasSectionFixed(QGraphicsItem):
    ...
    def paint(self, painter, option, widget=None):
        ...
        if self.hovered:
            g2 = QLinearGradient(btn_rect.topLeft(), btn_rect.bottomRight())
            g2.setColorAt(0, QColor(t["accent_bright"])); g2.setColorAt(1, QColor(t["accent"]))
...
class CanvasBlockSolid(QGraphicsItem):
    def __init__(self, pid, text, signals, zoom):
        ...
        self.zoom = zoom
        self.hovered = False
        self._font_text = QFont("Segoe UI", int(12 * zoom), QFont.Bold)
        self._calc_size()
        self.setAcceptHoverEvents(True)
    ...
    def _calc_size(self):
        ...
        if not self.text:
            ...
            text_rect = fm.boundingRect(0, 0, int(self.base_w - pad_x), 10000,
    ...
    def paint(self, painter, option, widget=None):
        ...
        if self.hovered:
            del_s    = 20 * self.zoom
            del_rect = QRectF(self.w - del_s - 4*self.zoom, 4*self.zoom, del_s, del_s)
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            del_s    = 22 * self.zoom
            del_rect = QRectF(self.w - del_s - 5*self.zoom, 5*self.zoom, del_s, del_s)
...
class PMCanvasWidget(QWidget):
    def __init__(self):
        ...
        self.sections_data = [
            ...
            {"id": "stk",  "t": "STAKEHOLDERS\nEXTERNOS", "st": "& Fatores externos", "i": "stk"},
    ...
    def reset_zoom(self): self.update_zoom(0.6)
    ...
    def _draw_board(self):
        self.scene.clear(); self.blocks_items.clear()
        z  = self.zoom_level
        cw = 450 * z; ch = 120 * z
        ...
        for sid, geo in grid_geo.items():
            ...
            sec_item = CanvasSectionFixed(sid, rect, s["t"], s["st"], s["i"], self.signals, z)
            ...
            if len(block_positions) > 1:
                ...
                for i in range(len(block_positions) - 1):
                    ...
                    c1 = QPointF(rect.x() + px1 + p_item1.w/2, rect.y() + py1 + p_item1.h/2)
                    c2 = QPointF(rect.x() + px2 + p_item2.w/2, rect.y() + py2 + p_item2.h/2)
                    self.scene.addLine(c1.x(), c1.y(), c2.x(), c2.y(), pen_conn).setZValue(5)
                    angle      = _math.atan2(c2.y()-c1.y(), c2.x()-c1.x())
                    arrow_size = 14 * z
                    mid_p      = QPointF((c1.x()+c2.x())/2, (c1.y()+c2.y())/2)
                    ap1 = QPointF(mid_p.x() - arrow_size*_math.cos(angle - _math.pi/6),
        ...
        self.scene.setSceneRect(-100*z, -100*z, cw*5 + 200*z, ch*10.5 + 200*z)

    def _on_add_block(self, sec_id):
        pid = f"b_{self.next_pid}"; self.next_pid += 1
    ...
    def _on_delete_block(self, pid):
        for sec_id, blocks in self.sections.items():
            for i, b in enumerate(blocks):
                if b["id"] == pid:
                    del blocks[i]; self._draw_board(); return

    def _on_edit_block(self, pid):
        if pid in self.blocks_items:
            p_item = self.blocks_items[pid]
            self._float_editor.open(pid, p_item.text, p_item.sceneBoundingRect(), self.view)

    def _on_commit_block(self, pid, text):
        for sec_id, blocks in self.sections.items():
            for b in blocks:
                if b["id"] == pid:
                    b["text"] = text; self._draw_board(); return
